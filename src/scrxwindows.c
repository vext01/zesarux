	...
	while (XCheckWindowEvent(dpy,ventana,KeyPressMask|KeyReleaseMask|StructureNotifyMask|ExposureMask|ButtonPressMask|ButtonReleaseMask|PointerMotionMask,&event)) {
	  //printf ("evento tipo: %d\n",event.type);
		...
		case ButtonPress:
			...
			debug_printf (VERBOSE_DEBUG,"Mouse Button press. x=%d y=%d. gunstick: x: %d y: %d", event.xbutton.x, event.xbutton.y,gunstick_x,gunstick_y);

		break;

                case ButtonRelease:
                        debug_printf (VERBOSE_DEBUG,"Mouse Button release. x=%d y=%d", event.xbutton.x, event.xbutton.y);
                        if ( event.xbutton.button == 1 ) {
				//mouse_left=0;
