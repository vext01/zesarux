...
int scrsdl_crea_ventana(void)
{
	...
	debug_printf (VERBOSE_DEBUG,"Creating window %d X %d",ancho,alto );
        ...
        if (ventana_fullscreen) {                                       // arvid: if fullscreen, maximize the window first
                SDL_VERSION(&info.version);                             // arvid: needed to get info
                SDL_GetWMInfo(&info);                                   // arvid: get the window info
                ShowWindow(info.window, SW_MAXIMIZE);                   // arvid: now maximize the window
                UpdateWindow(info.window);                              // arvid: refresh window

                ancho=screen_get_window_size_width_zoom_border_en();    // arvid: get new dims
                ancho +=screen_get_ext_desktop_width_zoom();            // arvid: get new dims
                alto=screen_get_window_size_height_zoom_border_en();    // arvid: get new dims

                flags |=SDL_FULLSCREEN;                                 // arvid: now set flag
                sdl_screen = SDL_SetVideoMode(ancho, alto, 32, flags);  // arvid: try to set fullscreen
        }
